	...
	<div id="website_deals">
		...
		WHILE($rows = mysql_fetch_array($result))
		{
			...
			<div class="deals1">
				...
				<a class="hoverdeals" href="/crop.php?cname='.$brand.'_deals&&ses='.$session.'&&urlo='.$url.'&aff_sub='.$aff_date.'" target="_blank"> Click to activate offer</a>
...
else
{
	...
	$result = mysql_query($q) or die("query");
	echo '
	<div style="background:white;min-height:2898px;margin-bottom:50px;border:2px solid #d0dde2;border: 2px solid #d0dde2;box-shadow: 0 2px 6px 2px rgba(131, 131, 131, 0.27);">
		<div class="allstore_body allstore_body2">
			<div class="top_merchants">
				<div class="merchants_header" align="left">Deals Stores</div>
			...
			WHILE(($rows = mysql_fetch_array($result))&&($count>=0))
			{
				...
				echo '
				<a href="/deals/'.$product_name.'"><div class="merchants_details_alls"><div id="dealimg'.$id.'" style="height: 97px;width:191px;"></div><div class="m_desertion">  </div><div class="merchants_text">Show Deals</div></div></a>
				';
			}
			echo '
			</div>
